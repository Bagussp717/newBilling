...
class PelangganTidakAktifController extends Controller
{
    ...
    {
        $pelanggansQuery = Pelanggan::whereNull('username_pppoe')->orWhere('username_pppoe', '');
        $cabangs = Cabang::all();
        $pakets = Paket::all();
        ...
        if ($user->hasRole('super-admin')) {
            ...
            $pelanggansQuery = Pelanggan::whereIn('kd_isp', $kd_isps)
        ...
        if ($request->wantsJson()) {
            // Use paginate for JSON response
            $pelanggans = $pelanggansQuery->paginate(10); // 10 data per page
            return response()->json([
                'pelanggan' => $pelanggans,
            ]);
        }
    
        // For view, fetch all data without pagination
    ...
    public function edit(Request $request, string $kd_pelanggan)
    {
